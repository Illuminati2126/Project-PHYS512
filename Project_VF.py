...
class Simulation_Electron :
    def __init__(self,folder,density,mean_free_time,relativeeps,timestep,minimalresidue, temperature,External_Electric_Field,External_Acceleration,ode="Euler",boundary="periodic"):
        ...
        self.mean_free_time=mean_free_time
    ...
    def diffusion(self):
        ...
        """
        self.vx=self.vx*np.exp(-self.timestep/self.mean_free_time)
        self.vy=self.vy*np.exp(-self.timestep/self.mean_free_time)
    ...
    def iteration(self):
        ...
        if self.ode=="Euler":
            for i in range(0,len(self.x)):
                ax[i], ay[i] = self.External_a(self.x[i],self.y[i],self.vx[i],self.vy[i],self.timestep,self.iterationcount) + constants.e/constants.m_e*(self.InternalElectrical(self.x[i],self.y[i])+self.External_E(self.x[i],self.y[i],self.iterationcount,self.timestep)) #calculated the acceleration for each one of the electrons
